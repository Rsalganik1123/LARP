...
class HTSAT_Swin_Transformer(nn.Module):
    ...
    def __init__(self, spec_size=256, patch_size=4, patch_stride=(4,4), 
        ...
        self.freq_ratio = self.spec_size // self.config['num_mel_bins']
        window = 'hann'
        center = True
        pad_mode = 'reflect'
        ref = 1.0
        ...
        self.logmel_extractor = LogmelFilterBank(sr=config['sample_rate'], n_fft=config['window_size'], 
            n_mels=config['num_mel_bins'], fmin=config['fmin'], fmax=config['fmax'], ref=ref, amin=amin, top_db=top_db, 
            freeze_parameters=True)
        # Spec augmenter
        self.spec_augmenter = SpecAugmentation(time_drop_width=64, time_stripes_num=2, 
            freq_drop_width=8, freq_stripes_num=2) # 2 2
        self.bn0 = nn.BatchNorm2d(self.config['num_mel_bins'])
    ...
    def forward(self, x: torch.Tensor, mixup_lambda = None, infer_mode = False, device=None):
        ...
        # x = x.unsqueeze(dim=1).to('cuda', non_blocking=True) # (batch_size, 1, time_steps, mel_bins) 
        x = x.transpose(1, 3) #torch.Size([5, 64, 1024, 1])
        x = self.bn0(x) #torch.Size([5, 64, 1024, 1])
        x = x.transpose(1, 3)
        if self.training:
        ...
        return output_dict['embedding'].unsqueeze(1)
...
def create_htsat_model(audio_cfg, enable_fusion=False, fusion_type='None'):    
    ...
    audio_ckpt = torch.load(audio_cfg['checkpoint'], map_location='cpu')
